...
module.exports = async function slackSend(core) {
  try {
    ...
    if (channelId.length > 0 && (message.length > 0 || payload)) {
      ...
      console.log(
        JSON.stringify({ channel: channelId, text: message, ...(payload || {}) }, null, 2)
      );
      // post message
      await web.chat.postMessage({ channel: channelId, text: message, ...(payload || {}) });
    } else {
      console.log(
        "missing either channel-id, slack-message or payload! Did not send a message via chat.postMessage with botToken"
      );
    }
